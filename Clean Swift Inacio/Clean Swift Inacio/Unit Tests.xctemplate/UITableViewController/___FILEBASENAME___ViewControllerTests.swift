...
class ___VARIABLE_sceneName___ViewControllerTests: XCTestCase {
	...
	var sutDelegate: ___VARIABLE_sceneName___TableViewDelegateMock!
	var window: UIWindow!
	...
	func setup___VARIABLE_sceneName___ViewController() {
		...
		sutDelegate = ___VARIABLE_sceneName___TableViewDelegateMock()
	...
	func loadView() {
		...
		let _ = sut.view
		sutDelegate.onRowSelected = sut.delegate?.onRowSelected
	...
	// MARK: - Row Selection On Table View

	func testTableViewDidSelectRowAtIndexPathMustCallDelegate() {
		// Given

		// When
		loadView()
		sutDelegate.tableViewDidSelectRowAtCompletionHandler = { (_ tableView: UITableView, _ indexPath: IndexPath) -> ___VARIABLE_sceneName___.UseCase.ViewModel.DisplayData in
			return ___VARIABLE_sceneName___.UseCase.ViewModel.DisplayData(title: "Title")
		}
		sutDelegate.tableView(UITableView(), didSelectRowAt: IndexPath(row: 0, section: 0))

		// Then
		XCTAssertTrue(sutDelegate.didCallTableViewDidSelectRowAt)
		XCTAssertTrue(sutDelegate.didCallOnRowSelected)
	}
